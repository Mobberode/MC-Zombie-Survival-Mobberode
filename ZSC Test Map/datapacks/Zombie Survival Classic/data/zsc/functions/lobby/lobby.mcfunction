###Restart Voting
function zsc:lobby/waiting/wait
execute if score #Score zsc.game.status matches 1 run scoreboard players set #Score zsc.game.status 2
function zsc:game/configapply
execute if score #Score zsc.game.status matches 2 run function zsc:difficulty/voting/vote
execute if score #Score zsc.game.status matches 2 run function zsc:game/skip/skip
#Restart Difficulty
scoreboard players set #Score zsc.difficulty -1
tag @a remove zsc.received
tag @a remove votedhard
tag @a remove votednormal
tag @a remove votedcustom
#Teams
team join Waiting @a

##Start plreffects
function zsc:lobby/plreffects
