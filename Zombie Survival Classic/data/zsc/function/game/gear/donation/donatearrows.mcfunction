...
$execute if score @s zsc.player.arrows.count >= @s zsc.player.arrows.give unless score #Score zsc.config.donate matches 0 run give @r[team=Alive] minecraft:arrow $(arrowgiveamount)
$execute unless score #Score zsc.config.donate matches 0 if entity @a[team=Alive] run clear @s minecraft:arrow $(arrowgiveamount)
