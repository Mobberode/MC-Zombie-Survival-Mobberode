...
##PVC on
execute unless score #Score zsc.config.player.vote.configs matches 0 unless entity @s[tag=voted.pms.true] run scoreboard players add #Score zsc.config.pms.on.votes 1
execute unless score #Score zsc.config.player.vote.configs matches 0 if entity @s[tag=voted.pms.false] run scoreboard players remove #Score zsc.config.pms.off.votes 1
