...
execute if entity @a[team=!Alive] run schedule function zsc:game/wave/restart 3s
...
execute if entity @a[team=!Alive] run tellraw @a ["",{"text":"Game Over! | Waves Survived: "},{"score":{"name":"#Score","objective":"zsc.wave"}}]
#Stop player check
...
execute if entity @a[team=!Alive] run schedule clear zsc:game/playercheck
